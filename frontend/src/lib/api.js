 ...
 * @param {Array<string>} options.addVideoIds - 要新增的影片ID陣列
 * @param {Array<string>} options.removeVideoIds - 要移除的影片ID陣列
 * @param {string} options.batchId - 當前批次ID（用於記錄來源）
 * @param {string} options.month - 月份（YYYY-MM格式）
 * @param {boolean} options.skipHistory - 是否跳過歷史記錄（自動保存時設為true）
 */
export async function updateCustomerList(customerId, options) {
  const response = await api.post(`/api/customer-list/${customerId}/update`, options)
  return response.data
}
 ...
 * 客戶提交清單（記錄歷史快照並發送通知）
 * @param {string} customerId - 客戶ID
 * @param {Object} changes - 變更摘要
 * @param {Array} changes.addedVideos - 新增的影片陣列
 * @param {Array} changes.removedVideos - 移除的影片陣列
 */
export async function submitCustomerList(customerId, changes) {
  const response = await api.post(`/api/customer-list/${customerId}/submit`, { changes })
  return response.data
}
 ...
 * 獲取客戶的提交歷史
 * @param {string} customerId - 客戶ID
 * @param {number} limit - 返回記錄數量限制（預設50）
 */
export async function getCustomerListHistory(customerId, limit = 50) {
  const response = await api.get(`/api/customer-list/${customerId}/history`, { 
    params: { limit } 
  })
  return response.data
