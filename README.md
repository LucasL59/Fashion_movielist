...
> **版本**：v3.5.2 ｜ **最後更新**：2026-01-07 ｜ **狀態**：✅ 可部署、可測試、已優化
...
- **⏰ 修復頁面閒置後無法操作的問題**：
  ...
  - 原因：Session 過期後 401 處理卡住，前端狀態未正確清除
  - 修復方案：
    - `signOut()` 改為立即執行，不等待 API 響應
    - 401 處理添加 5 秒超時保護，超時後強制跳轉
    - 新增頁面可見性監聽器，用戶回到頁面時主動檢查 Session
    - 使用 `window.location.replace()` 避免返回導航問題
    - Session 過期時清除所有認證相關的 localStorage

### v3.5.1 功能新增與修正 ✨

- **🏷️ 新影片標籤功能**：
  - 客戶選擇影片時，能明確看到哪些影片是**本月新增**的（相對於上個月）
  - 後端 API 自動比較當前月份與上個月份的影片清單
  - 每個新影片卡片左下角顯示閃爍的「✨ New」橘色漸層標籤
  - 頁面標題旁顯示「N 部新片」統計提示
  - 使用影片標題進行比對，確保跨批次識別準確

- **📧 客戶提交影片選擇通知修正**：
  - 前端顯示文字修正：「所有系統管理員與所有上傳者」
  - 後端預設通知對象修正：現在正確包含所有管理員 + 所有上傳者
  - 新增 `getAdminAndUploaderRecipients()` 函數確保實際郵寄行為與顯示一致
  - 修正原本只通知「該批次上傳者」的問題

- **🕐 時區修正（台灣時間）**：
  - 所有郵件和 Excel 匯出的時間戳記現在正確顯示台灣時間
  - 修正伺服器使用 UTC 時區導致時間差 8 小時的問題
  - 明確指定 `timeZone: 'Asia/Taipei'` 參數
  - 影響範圍：郵件通知時間、Excel 匯出提交時間

### v3.5.0 功能改進與修正 🚀

- **📢 新批次提示邏輯優化**：
  - 只要有新批次上傳，所有客戶都會看到「有新的影片清單可供您挑選」提示
  - 因應上傳者可能多次更新同月份清單的情境

- **🏷️ 調整記錄顯示來源月份**：
  - 客戶的「調整記錄」頁面展開詳情時，每部影片旁顯示來源月份標籤
  - 新增影片顯示藍色標籤，移除影片顯示紅色標籤

- **👤 Uploader 權限擴展**：
  - 上傳者現可查看「客戶清單總覽」頁面
  - 上傳者可查看管理員儀表板的「客戶清單調整明細」
  - 後端 API 添加 `requireRoles(['admin', 'uploader'])` 權限控管

- **📊 月份調整彙總匯出功能**：
  - 新增「匯出調整記錄」按鈕於客戶清單總覽頁面
  - 可選擇月份並匯出 Excel 檔案
  - Excel 包含三個工作表：調整摘要、新增影片明細、移除影片明細
  - 每個明細包含來源月份資訊，方便後續處理

- **🔧 郵件管理預設通知對象修正**：
  - 「新影片清單上傳」的預設通知對象現正確顯示：系統管理員 + 所有客戶
  - 修正原本錯誤顯示為「管理員 + 上傳者」的問題
...
- **📜 提交歷史記錄**：
  - 顯示所有提交記錄（最新在上）
  - 每筆記錄顯示：提交時間、總數、新增數量、移除數量
  - 可展開查看變更明細（新增/移除的影片列表）
  - 最新一筆標記「最新」標籤
